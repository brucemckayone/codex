<script lang="ts">
	...
	const statusConfig = {
		404: {
			title: m.org_error_not_found(),
			description: m.org_error_not_found_description(),
			icon: 'search',
		},
		401: {
			title: m.org_error_unauthorized(),
			description: m.org_error_unauthorized_description(),
			icon: 'lock',
		},
		403: {
			title: m.org_error_forbidden(),
			description: m.org_error_forbidden_description(),
			icon: 'lock',
		},
		500: {
			title: m.org_error_server_error(),
			description: m.org_error_server_error_description(),
			icon: 'warning',
		},
	};
	...
	const config = $derived(
		statusConfig[page.status] ?? {
			title: m.org_error_unknown(),
			description: m.org_error_unknown_description(),
			icon: 'warning',
		}
	);
	...
	const homeUrl = $derived(`/org/${page.params.slug}`);
</script>

<svelte:head>
	<title>{page.status} {config().title} | Content</title>
</svelte:head>
	...
	<div class="error-card">
		...
		<div class="error-actions">
			{#if page.status === 401 || page.status === 403}
				<a href="/login" class="btn btn-primary">{m.org_error_sign_in()}</a>
			{:else}
				<a href={homeUrl} class="btn btn-primary">{m.org_error_go_home()}</a>
			{/if}
