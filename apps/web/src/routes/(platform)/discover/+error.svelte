<script lang="ts">
	...
	const statusConfig = {
		404: {
			title: m.content_error_not_found(),
			description: m.org_error_not_found_description(),
			icon: 'search'
		},
		500: {
			title: m.content_error_server(),
	...
	const config = $derived(
		statusConfig[$page.status] ?? {
			...
			description: $page.error?.message ?? m.org_error_unknown_description(),
			icon: 'warning'
		}
	);
...
<svelte:head>
	<title>{$page.status} {config().title} | Discover</title>
</svelte:head>
...
<div class="error-page" role="alert" aria-live="polite">
	<div class="error-card">
		...
		<div class="error-actions">
			<a href="/discover" class="btn btn-primary">{m.org_error_go_home()}</a>
