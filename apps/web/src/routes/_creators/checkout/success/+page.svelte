<!--
  ...
  Post-checkout landing page for creator content purchases.
  Shows purchase confirmation when verified, or graceful fallback if pending.
-->
<script lang="ts">
  ...
  const contentUrl = $derived(
    username && contentSlug
      ? `/${username}/content/${contentSlug}`
      : username
        ? `/${username}/content`
        : '/'
  );
...
<svelte:head>
  <title>{m.checkout_success_title()} | Creators</title>
</svelte:head>
...
<style>
  ...
  .checkout-success__icon--success { color: var(--color-success-600); }
  .checkout-success__icon--pending { color: var(--color-text-secondary); }
