...
<svelte:head>
  <title>{m.studio_error_title()} | My Studio</title>
</svelte:head>
...
<div class="error-page" role="alert" aria-live="polite">
  <div class="error-container">
    ...
    <h1 class="error-title">{m.studio_error_title()}</h1>
    <p class="error-description">{m.studio_error_description()}</p>
...
<style>
  .error-page { min-height: 50vh; display: flex; align-items: center; justify-content: center; padding: var(--space-4); }
  .error-container { width: 100%; max-width: 480px; text-align: center; display: flex; flex-direction: column; align-items: center; gap: var(--space-3); }
  ...
  .error-description { font-size: var(--text-sm); color: var(--color-text-secondary); margin: 0; }
