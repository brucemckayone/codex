<!--
  ...
  for paid content. Mirrors the org content detail page pattern.
-->
<script lang="ts">
...
<svelte:head>
  <title>{data.content.title} | {creatorName}</title>
...
<style>
  ...
  .content-detail__creator-link {
    color: var(--color-text-secondary);
    text-decoration: none;
    transition: var(--transition-colors);
  }

  .content-detail__creator-link:hover {
  ...
  .prose :global(ul), .prose :global(ol) { padding-left: var(--space-6); margin: var(--space-2) 0; color: var(--color-text-secondary); }
