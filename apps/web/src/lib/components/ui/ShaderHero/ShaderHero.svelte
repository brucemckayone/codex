  ...
  onMount(() => {
    ...
    async function loadRenderer(preset: ShaderPresetId): Promise<ShaderRenderer | null> {
      switch (preset) {
        ...
        case 'pulse': {
          const { createPulseRenderer } = await import('./renderers/pulse-renderer');
        ...
        case 'nebula': {
          const { createNebulaRenderer } = await import('./renderers/nebula-renderer');
          return createNebulaRenderer();
        }
        case 'turing': {
          const { createTuringRenderer } = await import('./renderers/turing-renderer');
        ...
        case 'glass': {
          const { createGlassRenderer } = await import('./renderers/glass-renderer');
