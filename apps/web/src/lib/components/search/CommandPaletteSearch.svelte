<script lang="ts">
	...
	let { scope, orgSlug, open = $bindable(false) }: Props = $props();
	...
	interface SearchCreatorResult {
		name: string;
		avatarUrl: string | null;
	}

	let results = $state<{ content: SearchContentResult[]; creators: SearchCreatorResult[] }>({
		content: [],
		creators: [],
	});
	...
	type FlatItem =
		| { type: 'recent'; term: string; index: number }
		| { type: 'content'; item: SearchContentResult; index: number }
		| { type: 'creator'; item: SearchCreatorResult; index: number };

	const allItems = $derived<FlatItem[]>([
		...(!query.trim()
			? recentSearches.map((term, i) => ({ type: 'recent' as const, term, index: i }))
			: []),
		...(query.trim()
			? results.content.map((item, i) => ({ type: 'content' as const, item, index: i }))
			: []),
		...(query.trim()
			? results.creators.map((item, i) => ({ type: 'creator' as const, item, index: i }))
			: []),
	]);
	...
	function selectItem(item: FlatItem) {
		if (item.type === 'recent') {
			...
			saveRecent(query.trim());
			open = false;
			goto(`/creators/${encodeURIComponent(item.item.name)}`);
		}
	}
	...
	// Flat index offsets for grouped results
	function contentFlatIndex(i: number): number {
		return i;
	}

	function creatorFlatIndex(i: number): number {
		return results.content.length + i;
	}
</script>
			...
			{#if !query.trim() && recentSearches.length > 0}
				<div class="palette__group">
					<div class="palette__group-header">
						...
						<button class="palette__group-clear" onclick={clearRecent}
							>{m.search_clear_button()}</button
						>
					</div>
			...
			{#if query.trim() && !loading}
				{#if results.content.length > 0}
					<div class="palette__group">
						...
						{#each results.content as item, i (item.id)}
							{@const flatIdx = contentFlatIndex(i)}
							<button
								class="palette__item palette__item--content"
								class:palette__item--active={activeIndex === flatIdx}
								role="option"
							...
							>
								...
								{:else}
									<div class="palette__thumb palette__thumb--empty"></div>
								{/if}
				...
				{#if results.creators.length > 0}
					<div class="palette__group">
						...
						{#each results.creators as creator, i (creator.name)}
							{@const flatIdx = creatorFlatIndex(i)}
							<button
