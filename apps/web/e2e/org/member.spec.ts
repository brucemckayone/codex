...
test.describe
  .serial('Organization Member - Fixtures', () => {
    ...
    test('can create multiple members with different roles in same org', async ({
      ...
      test.slow(); // Creates 4 members via HTTP, takes longer
      const timestamp = Date.now();
    ...
    test('can add member to existing organization', async ({
      ...
      expect(newMember.organization.slug).toContain(
        `existing-org-${timestamp}`
      );
...
test.describe
  .serial('Organization Page - With Authenticated Member', () => {
    ...
    test('can use createOrgWithMembers for complex scenarios', async ({
      ...
      test.slow(); // Creates 3 members via HTTP, takes longer
      const timestamp = Date.now();
      // Create a content team with different roles
      const { organization, memberContexts } = await createOrgWithMembers(
        {
          name: `Content Team ${timestamp}`,
          slug: `content-team-${timestamp}`,
        },
        [
          {
