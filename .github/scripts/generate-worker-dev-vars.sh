...
# WORKER_SHARED_SECRET is required by workers that gate /internal HMAC routes
# (notifications-api env validation; revenue-share worker-to-worker calls).
# Falls back to BETTER_AUTH_SECRET so existing env supplies it without changes.
cat > "${VARS_FILE}" << EOF
...
EOF
  ...
  "organization-api")
    ...
    # STRIPE_SECRET_KEY, /api/organizations/:id/tiers throws 500 and the
    # org layout's loadOrgTiers() cascades into Org Homepage 500s.
    cat >> "${VARS_FILE}" << EOF
STRIPE_SECRET_KEY=${STRIPE_SECRET_KEY}
EOF
    ;;
