...
# WORKER_SHARED_SECRET: every worker uses HMAC for inter-worker calls or
# token signing (notifications-api unsubscribe tokens). Missing this causes
# procedure() worker-auth and token verification to throw 500 instead of the
# expected 400/401/429 — see ~/.claude/memory/feedback_worker_shared_secret_dev_vars.md.
cat > "${VARS_FILE}" << EOF
...
WORKER_SHARED_SECRET=${WORKER_SHARED_SECRET:-test-worker-shared-secret}
EOF
