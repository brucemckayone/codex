...
import { adminFixture, authFixture } from '../fixtures';
import {
  expectErrorResponse,
  expectForbidden,
...
test.describe('Admin Dashboard', () => {
  ...
  test.describe('Authentication & Authorization', () => {
    ...
    test('should reject non-platform-owner users', async ({ request }) => {
    ...
    test('should accept platform_owner user', async ({ request }) => {
      ...
      const stats = await adminFixture.getRevenueStats(request, admin.cookie);
  ...
  test.describe('Revenue Analytics', () => {
    test('should return zero stats for new org', async ({ request }) => {
      test.setTimeout(60000);
    ...
    test('should calculate revenue from completed purchases', async ({
      request,
    }) => {
    ...
    test('should scope to platform owner org only', async ({ request }) => {
  ...
  test.describe('Customer Analytics', () => {
    test('should count distinct customers', async ({ request }) => {
      test.setTimeout(180000);
      ...
      const content1Response = await request.post(
        `${WORKER_URLS.content}/api/content`,
        {
          headers: {
            Cookie: creatorCookie,
      ...
      const media2Response = await request.post(
        `${WORKER_URLS.content}/api/media`,
        {
          headers: {
            Cookie: creatorCookie,
      ...
      const content2Response = await request.post(
        `${WORKER_URLS.content}/api/content`,
        {
          headers: {
            Cookie: creatorCookie,
      ...
      // Purchase 1
      const checkout1 = await request.post(
        `${WORKER_URLS.ecom}/checkout/create`,
        {
          headers: {
            Cookie: buyerCookie,
      ...
      // Purchase 2
      const checkout2 = await request.post(
        `${WORKER_URLS.ecom}/checkout/create`,
        {
          headers: {
            Cookie: buyerCookie,
  ...
  test.describe('Content Management', () => {
    test('should list all org content with pagination', async ({ request }) => {
      ...
      for (let i = 0; i < 3; i++) {
        await request.post(`${WORKER_URLS.content}/api/content`, {
          headers: {
            Cookie: creatorCookie,
            'Content-Type': 'application/json',
          },
      ...
      const result = await adminFixture.listAllContent(request, admin.cookie, {
        page: 1,
        limit: 2,
      });
    ...
    test('should publish draft content', async ({ request }) => {
      test.setTimeout(120000);
    ...
    test('should unpublish published content', async ({ request }) => {
      test.setTimeout(120000);
    ...
    test('should soft delete content', async ({ request }) => {
      test.setTimeout(120000);
    ...
    test('should reject cross-org content operations', async ({ request }) => {
      test.setTimeout(120000);
  ...
  test.describe('Customer Management', () => {
    test('should list customers with aggregated stats', async ({ request }) => {
      test.setTimeout(180000);
    ...
    test('should get customer details with purchase history', async ({
      request,
    }) => {
      test.setTimeout(180000);
    ...
    test('should grant complimentary access', async ({ request }) => {
      test.setTimeout(180000);
      ...
      const media1Response = await request.post(
        `${WORKER_URLS.content}/api/media`,
        {
          headers: {
            Cookie: creatorCookie,
