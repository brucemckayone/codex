...
- Platform assets (thumbnails, waveforms, logos)

> **Implementation Reference**: See [`P1-TRANSCODE-001-implementation-plan.md`](../roadmap/implementation-plans/P1-TRANSCODE-001-implementation-plan.md) for the `paths.ts` module that generates these R2 keys programmatically.
...
│   └── hls-audio/
│       └── {mediaId}/
│           ├── master.m3u8               # HLS audio master playlist
│           ├── 128k/
│           │   ├── playlist.m3u8         # 128kbps AAC variant
│           │   └── segment_*.ts
│           └── 64k/
│               ├── playlist.m3u8         # 64kbps AAC variant
│               └── segment_*.ts
│
├── {creatorId-2}/
│   ├── originals/
...
**Transcoding Output Paths** (Quick Reference):

| Output Type | Bucket | Key Pattern |
|-------------|--------|-------------|
| Original upload | media | `{creatorId}/originals/{mediaId}/original.{ext}` |
| Video HLS master | media | `{creatorId}/hls/{mediaId}/master.m3u8` |
| Video variant | media | `{creatorId}/hls/{mediaId}/{quality}/playlist.m3u8` |
| Video preview | media | `{creatorId}/hls/{mediaId}/preview/preview.m3u8` |
| Audio HLS master | media | `{creatorId}/hls-audio/{mediaId}/master.m3u8` |
| Audio variant | media | `{creatorId}/hls-audio/{mediaId}/{bitrate}/playlist.m3u8` |
| Video thumbnail | assets | `{creatorId}/thumbnails/media/{mediaId}/auto-generated.jpg` |
| Audio waveform JSON | assets | `{creatorId}/waveforms/{mediaId}/waveform.json` |
| Audio waveform PNG | assets | `{creatorId}/thumbnails/media/{mediaId}/waveform.png` |

> **Note**: These paths are generated by `packages/transcoding/src/paths.ts` - see implementation plan.
...
│   │   └── media/
│   │       └── {mediaId}/
│   │           ├── auto-generated.jpg    # Video thumbnail (10% mark)
│   │           └── waveform.png          # Audio waveform image (1280x720)
│   ├── waveforms/
│   │   └── {mediaId}/
│   │       └── waveform.json             # Audio waveform data (10 pts/sec)
│   └── branding/
...
6. Backend creates `media_items` record:
   ...
   ```
7. **Phase 1**: Backend triggers media-api transcoding job (no queue in Phase 1)
...
- **[Media Transcoding Implementation Plan](../roadmap/implementation-plans/P1-TRANSCODE-001-implementation-plan.md)** - Complete transcoding implementation with `paths.ts` module
...
1. **Thumbnail Auto-Generation**: ✅ **RESOLVED**
   - Extract frame at 10% mark during transcoding
   - Store at `{creatorId}/thumbnails/media/{mediaId}/auto-generated.jpg`
   - See implementation plan for FFmpeg command

2. **Audio Waveforms**: ✅ **RESOLVED**
   - Generate JSON waveform (10 points/sec, 8-bit) at `{creatorId}/waveforms/{mediaId}/waveform.json`
   - Generate PNG waveform image (1280x720) at `{creatorId}/thumbnails/media/{mediaId}/waveform.png`
   - See implementation plan for audiowaveform commands

3. **Video Previews**: ✅ **RESOLVED**
   - Generate 30-second preview HLS at 720p
   - Store at `{creatorId}/hls/{mediaId}/preview/preview.m3u8`

---

## Open Questions

1. **Offering Recordings**: How do we handle live offering recordings (e.g., recorded Zoom sessions)?
   - **Recommendation**: Store in `codex-media-production/{creatorId}/originals/{mediaId}/` (treat as media item with `source_type = 'offering_recording'`)

2. **Resource Versioning**: Should we support multiple versions of same resource (e.g., workbook-v1.pdf, workbook-v2.pdf)?
   - **Proposed**: Store as separate resources, use `attachment_label` to indicate version
...
**Changes**:
- v2.1: Added transcoding outputs (preview HLS, waveforms), resolved open questions, linked implementation plan
- v2.0: Updated from per-creator buckets to unified buckets with per-creator folder structure
