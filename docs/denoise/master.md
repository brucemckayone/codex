...
| `types:as-cast-without-guard` | **3** | 2026-04-26 | 2026-04-26 | **🚨 R7 STANDARD 3-HIT THRESHOLD REACHED iter-028** — promotion queued for iter-029 prep. iter-028 F5 = DatabaseTransaction cast at content-service.ts:158 exposed by Tier 6.A widening. Suggested rule: type assertions narrowing source type MUST be guarded by runtime check, with explicit exception list (Drizzle bridges, framework defaults, Proxy targets). (Codex-lqvw4.9, .12, Codex-qmrwe) |
...
- **🎯 Round 2 progress (after iter-028)**: First pass complete (12/12). Second pass: 1 cell at 2/3 (types × apps/web), 1 cell at 1/3 (performance × packages), 2 cells reset to 0/3 (simplification × apps/web, types × packages). Round 3 R-rule effectiveness verified for: schema .extend ✅, SubscriptionContext ✅, R14 fanout ✅, R12 ✅ (8 sites), R11 Database/RevenueSplit/Logger/orgmanagement ✅ (4 of 5 P0/P1 closed), STUDIO_ROLES ⚠️ Header-only.
  - Round 1 (iter-001 → iter-012): 72 findings catalogued
  - Round 2 first pass (iter-013 → iter-024): 0 findings
  - Round 2 second pass (iter-025 → iter-028): 11 findings total (1 R12, 3 simplification major, 1 doc-rot increment, 4 new R11 inline-shape, 1 R7-trigger DatabaseTransaction cast, 1 lqvw4.1 traceability pointer)
- **🚨 R-RULE PROMOTIONS QUEUED FOR iter-029 PREP**:
  1. **`types:as-cast-without-guard` (R7 STANDARD 3-HIT THRESHOLD)** — hits=3 across iter-005/006/028. Suggested R15: "Type assertions of the form `value as Foo` where Foo is more specific than the source type MUST be guarded by a runtime check (Zod schema, type guard, or instanceof) — except for: (a) Drizzle infinite-recursion bridges, (b) framework defaults like `{} as TPolicy`, (c) Proxy-target placeholders. Verified by a grep + manual review per cycle." Exception list is non-trivial — recommend drafting first, human-reviewing before applying to §1.
  2. **`simplification:dup-content-item-shape` (2-HIT EARLY PROMOTION CANDIDATE)** — cumulative cycle_density=5 across 2 cycles, mirrors R14's profile. Suggested R16: "Structural shapes consumed across ≥2 component-family files MUST resolve to a single declaration site (sibling `types.ts`, package barrel, or generated wire shape via `NonNullable<...>`); inline `interface`/`type` redeclarations exactly matching an exported sibling are blocker."
- **Suggested next cell** (§5.0 sort): tied top tier — types × packages (iter-028, 10 findings, 0/3 — JUST RAN), simplification × apps/web (iter-027, 11 findings, 0/3 — JUST RAN), performance × packages (iter-026, 9 findings, 1/3 — JUST RAN), types × apps/web (iter-025, 8 findings, 2/3). Excluding just-run cells, `last_run ASC` and prioritising types/security → either:
  - **`types × apps/web` (iter-029)** to confirm fidelity at 3/3 (no apps/web type churn since iter-025 likely; would close out one cell)
  - **`security × apps/web` (iter-029)** — Tier 7.A landed CSP header (R10 effectiveness check) and HSTS work. Highest-signal security audit.
  - **`security × workers` (iter-029)** — workers/auth/middleware/rate-limiter.ts has user's in-progress R9 fix (uncommitted). Worth a focused audit when committed.
  - Recommend `security × apps/web` for iter-029 to verify R10 (CSP) effectiveness — that's the highest-value drift-detection target.
